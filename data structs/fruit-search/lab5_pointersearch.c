...
pointerComparator_LatinName(const void *vKey, const void *vData)
{
	...
	key  = (char *)vKey; //typecasting to get the data from the function call arguments
	...
	char *temp = (*fruitData)->latin; //accessing from the structure
	result = strcmp(key, temp); //comparing if the temporary one iterating through the list is the same as the argument it is looking for. 
...
searchInLatinName(FruitData * const *fruitDataPointers, int nFruits, char *key)
{
	...
	pointerResult = (FruitData **)binarysearch(key, fruitDataPointers, nFruits, sizeof(FruitData *), pointerComparator_LatinName);
